chrome.action.onClicked.addListener(async (tab) => {
  ...
  // Spoof User-Agent as a search engine bot
  const SPOOFED_USER_AGENT = "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)";
  
  chrome.webRequest.onBeforeSendHeaders.addListener(
    (details) => {
      for (let header of details.requestHeaders) {
        if (header.name.toLowerCase() === "user-agent") {
          header.value = SPOOFED_USER_AGENT;
          break;
        }
      }
